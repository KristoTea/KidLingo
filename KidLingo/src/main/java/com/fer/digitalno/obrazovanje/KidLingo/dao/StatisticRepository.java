...
public interface StatisticRepository extends JpaRepository<Statistic, Long> {
    ...
    @Query(value = "SELECT number_of_correct_answers FROM statistic WHERE language = ?1 AND level = ?2 AND category = ?3", nativeQuery = true)
    List<Integer> findAllNumberOfCorrectAnswersByLanguageAndLevelAndCategory(String language, String level, String category);

    @Query(value = "SELECT solving_speed FROM statistic WHERE language = ?1 AND level = ?2 AND category = ?3", nativeQuery = true)
